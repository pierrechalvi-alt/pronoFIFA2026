...
const LIVE_SCORES_ENABLED = isLiveScoresEnabled();
...
function isLiveScoresEnabled(){
  const query = new URLSearchParams(window?.location?.search || "").get("fwc26Live");
  const explicitMeta = document.querySelector('meta[name="fwc26-live-enabled"]')?.content;
  const explicitGlobal = typeof window !== "undefined" ? window.__FWC26_LIVE_ENABLED__ : null;
  const raw = String(query || explicitMeta || explicitGlobal || "").trim().toLowerCase();
  return raw === "1" || raw === "true" || raw === "yes" || raw === "on";
}
...
function setupLiveScoresSync(){
  ...
  if (!liveApi) return;
  pullLiveScores(liveApi).catch((err) => {
    console.warn("Live scores indisponibles (non bloquant) :", err?.message || err);
  });
  setInterval(() => {
